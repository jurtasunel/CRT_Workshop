...
for (element in vector){
  if (element > 50){
    above_50 <- c(above_50, element)
  } else{
    below_50 <- c(below_50, element)
  }
} 

# [3] 
# Print the number of items in above_50 and below_50
print(length(above_50))
print(length(below_50))

# [4] 
# Find the fraction of items that are above 50 & below 50 (using the original vector as the denominator)
length(above_50) / length_of_vector
length(below_50) / length_of_vector
```
...
age <- as.Date(readline(prompt = "Please Enter Your Date of Birth \nUse format YYYY-MM-DD: "))
...
expression_ratio_CXCL12 <- CXCL12_treatment / CXCL12_control

# [3]
# Print the expression ratio using print() and paste0()
# Insert "The expression ratio for CXCL12 is: "
print(paste0("The expression eatio for CXCL12 is: ", expression_ratio_CXCL12))
...
CXCL12_inverse <- -1 / expression_ratio_CXCL12
...
CXCL12_log2 <- log2(expression_ratio_CXCL12)
...
for (element in treatment){
  i <- i + 1
  print(paste0("Gene_", i, " has an expression value of ", element))
}
```
...
for(index in 1:length(treatment)){
  treatment_value <- treatment[index]
  control_value <- control[index]
  print(paste0("Gene_", index," has an expression value of ", treatment_value, " in the treatment sample, and an expression value of ", control_value, " in the control sample"))
}
```
...
for (index in 1:length(treatment)){
  ...
  print(paste0("Gene_", index, " expression ratio is ", ratio))
}
